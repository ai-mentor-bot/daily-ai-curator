...
REVOKE ALL ON TABLE daily_ai_curations_v2 FROM PUBLIC;
...
-- 内部キュレーションデータは service role のみ読み取り可能
...
-- 外部ユーザーによるデータ汚染を防ぐため service role のみ挿入可能
...
REVOKE ALL ON TABLE monthly_learning_reports FROM PUBLIC;
...
REVOKE ALL ON TABLE monthly_learning_summary FROM PUBLIC;
...
REVOKE ALL ON TABLE risk_factor_analysis FROM PUBLIC;
...
REVOKE ALL ON TABLE implementation_analysis FROM PUBLIC;
...
REVOKE ALL ON TABLE confidence_distribution FROM PUBLIC;
