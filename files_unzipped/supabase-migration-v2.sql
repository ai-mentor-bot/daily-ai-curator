...
-- 内部ジョブのみ読み取り可能（thinking/process データを含むため）
DROP POLICY IF EXISTS "allow_select_v2" ON daily_ai_curations_v2;
DROP POLICY IF EXISTS "allow_service_role_select_v2" ON daily_ai_curations_v2;
CREATE POLICY "allow_service_role_select_v2" ON daily_ai_curations_v2
  FOR SELECT TO service_role USING (true);

-- 内部ジョブのみ挿入可能（公開 API からのデータ汚染を防止）
DROP POLICY IF EXISTS "allow_insert_v2" ON daily_ai_curations_v2;
DROP POLICY IF EXISTS "allow_service_role_insert_v2" ON daily_ai_curations_v2;
CREATE POLICY "allow_service_role_insert_v2" ON daily_ai_curations_v2
...
DROP POLICY IF EXISTS "allow_service_role_select_monthly_reports" ON monthly_learning_reports;
CREATE POLICY "allow_service_role_select_monthly_reports" ON monthly_learning_reports
  FOR SELECT TO service_role USING (true);

DROP POLICY IF EXISTS "allow_insert_monthly_reports" ON monthly_learning_reports;
DROP POLICY IF EXISTS "allow_service_role_insert_monthly_reports" ON monthly_learning_reports;
CREATE POLICY "allow_service_role_insert_monthly_reports" ON monthly_learning_reports
  FOR INSERT TO service_role WITH CHECK (true);
