...
test("internal report tables are restricted to the service role", () => {
  for (const table of INTERNAL_TABLES) {
    assert.match(
      migration,
      new RegExp(`CREATE POLICY "service_role_select[^"]*" ON ${table}\\s+FOR SELECT TO service_role\\s+USING \\(auth\\.role\\(\\) = 'service_role'\\);`, "s"),
      `${table} must only allow service-role reads`,
    );
    assert.match(
      migration,
      new RegExp(`CREATE POLICY "service_role_insert[^"]*" ON ${table}\\s+FOR INSERT TO service_role\\s+WITH CHECK \\(auth\\.role\\(\\) = 'service_role'\\);`, "s"),
...
test("migration does not create broad public report policies", () => {
  assert.doesNotMatch(migration, /FOR\s+(SELECT|INSERT)[\s\S]*?(USING|WITH CHECK)\s*\(true\)/i);
  assert.doesNotMatch(migration, /GRANT\s+(?:ALL|SELECT|INSERT|UPDATE|DELETE)[^;]+TO\s+(?:anon|authenticated)\b/i);
});
